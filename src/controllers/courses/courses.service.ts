...
export class CoursesService {
  ...
  async createCourseReview(
    ...
    if (!enrolled) {
      throw new ConflictException(
        "User must be enrolled to the course to post a review",
      )
    }
    ...
    if (reviewed) {
      throw new ConflictException(
        "This user already has a review on this course",
      )
    }
  ...
  async createCourseCertificate(
    ...
    if (found) {
      throw new ConflictException(
        "This user already get certificate of this course",
      )
    }
  ...
  async giftCourse(input: GiftCourseInput): Promise<GiftCourseOutput> {
    ...
    await queryRunner.startTransaction()
    try {
      const receiveUser = await this.accountMySqlRepository.findOneBy({
        email: receiveUserEmail,
      })

      if (!receiveUser) {
        throw new NotFoundException(
          "Receive user not found or they aren't a CiStudy member",
        )
      }
      ...
      if (enrolled) {
        throw new ConflictException(
          "This user is already enrolled to this course",
        )
      }
